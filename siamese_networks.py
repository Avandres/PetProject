...
class SiameseConv2d:
    ...
    def __build_model(self) -> Model:
        ...
        """
        first_input = Input(shape=self.__input_shape)
        second_input = Input(shape=self.__input_shape)
        ...
        return Model(inputs=[first_input, second_input], outputs=distance)
...
class SiameseLSTM():
    ...
    ### Всё до этого - в абстрактный класс
    def __build_base_network(self) -> Sequential:
    ...
    # И отсюда тоже можно в абстрактный класс добавить
    def load_weights(self, path_to_weights: str = "default_weights.ckpt"):
        self.model.load_weights(path_to_weights)
